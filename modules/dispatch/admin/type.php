...
{
    global $db, $module_data,$admin_info,$lang_module;

    $sql = "DELETE FROM `" . NV_PREFIXLANG . "_" . $module_data . "_document` WHERE `type`=" . $typeid;
 	$db->query($sql);

	$sql = "DELETE FROM `" . NV_PREFIXLANG . "_" . $module_data . "_rows` WHERE `idtype`=" . $typeid;
 	$db->query($sql);
    ...
    $db->query($sql);
    nv_insert_logs(NV_LANG_DATA, $module_data,$lang_module['type_de'], '', $admin_info['userid'], '');
}
...
if ($nv_Request->isset_request('edit', 'get')) {
    ...
    if ($nv_Request->isset_request('submit', 'post')) {
        ...
        if (!$is_error) {
            ...
            $query = "UPDATE `" . NV_PREFIXLANG . "_" . $module_data . "_type` SET
                		`alias`=" . $db->quote(change_alias($array['title']). "-" . $typeid) . " WHERE `id`=" . $typeid;
            $db->query($query);
